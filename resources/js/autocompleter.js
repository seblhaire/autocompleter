      ...
      if (data.res.length > 0){
        for (var i = 0; i < data.res.length; i++){
          ...
          let li = jQuery('<li></li>')
              ...
              .on('mouseover', function(){ jQuery(this).addClass(self.options.activeitem)})
              .on('mouseout', function(){ jQuery(this).removeClass(self.options.activeitem)});
...
(function(jQuery) {
	...
	jQuery.fn.sebautocompleter = function(url, options)  {
		return this.each(function() {
			...
			if (element.data('sebautocompleter')) return element.data('sebautocompleter');
